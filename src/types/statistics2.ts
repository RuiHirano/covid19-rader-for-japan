...
export class StatsCalculator {
	...
	GraphResult: Graphs
	Items: Item[]
	ItemsObj: { [key: number]: { [key: number]: { [key: number]: number } } }
	Param: Content
	constructor(items: Item[], param: Content) {
		this.StatsResult = new Statistics()
		this.GraphResult = new Graphs()
		this.Items = items
		this.Param = param
		this.ItemsObj = this.createItemObj()
	}
	getGraphResult() {
	...
	createItemObj() {
		// 三次元配列 obj[year][month][day] = profit
		let obj: { [key: number]: { [key: number]: { [key: number]: number } } } = {}
		this.Items.forEach((item: Item) => {
			//const date: Moment = item.StartDate
			const date: Moment = moment()
			console.log("date: ", date.year(), date.month(), date.date())
			// init to avoid undefined error
			if (obj[date.year()] === undefined) {
				obj[date.year()] = {}
				if (obj[date.year()][date.month()] === undefined) {
					obj[date.year()][date.month()] = {}
					if (obj[date.year()][date.month()][date.date()] === undefined) {
						obj[date.year()][date.month()][date.date()] = 0
					}
				}
			}
			obj[date.year()][date.month()][date.date()] += item.Profit
		})
		return obj
	}

	calcStats(period: PeriodType) {

		for (let ykey in this.ItemsObj) {
			const monthObj = this.ItemsObj[ykey]
			for (let mkey in monthObj) {
				const dayObj = monthObj[mkey]
				for (let dkey in dayObj) {
					const dayProfit = dayObj[dkey]
					console.log("obj", ykey, mkey, dkey, dayProfit)
				}
			}
		}
		...
		switch (period) {
			case PeriodType.YEAR:
				for (let key in this.ItemsObj) {
					console.log("obj", key, this.ItemsObj[key])
				}
				break;
